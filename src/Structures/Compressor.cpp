...
void Compressor::attach(DataPacket packet)
{
	...
	if (generateFormat) format += String(startIndex) + "_" + String(index) + "_" + packet.name + "_" +
								  String(packet.bottomLimit, packet.decimals) + "_" + String(packet.upperLimit, packet.decimals) + "_" + String(packet.decimals) + " ";
...
void Compressor::download(char name, float& variable)
{
	...
	if (retrieved.name != 'I') variable = retrieved.value;
}
...
DataPacket Compressor::find(char name, int& startBit, int& endBit)
{
	...
	if (format[ind] == 'X')
	{
		...
		return DataPacket('I', 0, 0, 0, 0);
	}
	startBit = current[0].toInt();
	endBit = current[1].toInt();
	return DataPacket(current[2][0], current[3].toFloat(), current[4].toFloat(), current[5].toInt(), -1);
}